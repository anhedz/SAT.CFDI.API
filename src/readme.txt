...
IApiService api = new ApiServiceBuilder()
						.Configuration(IConfiguration)
						.Solicitante(ISolicitante)
						.Build();

IConsulta consulta = new ApiConsultaServiceBuilder()
							.Tipo(TipoEnum)
...
api.Autentica().Execute();
if (api.IsAuthenticated) {
	var response = api.Consulta(ISolicitud).Execute();
} else { 
   Console.Write(api.CodigoStatus);
   Console.Write(api.CodigoMessage);
}


api.Autentica().AddRequest(IConsulta).Execute();

if (api.IsAccepted) {
	Console.WriteLine(api.GetResponse().IdSolicitud));
} else { 
   Console.Write(api.CodigoStatus);
   Console.Write(api.CodigoMessage);
}
...
if (api.IsAuthenticated) {
	var verifica = api.Verifica(IdSolicitud).Execute();
}

var descarga = api.Descarga(IdSolicitud).PathDownload(Path).Execute();


Si desea consultar el mismo rango de fechas, modifique los minutos o segundos para que se tome como diferente, ya que el SAT limita las descargas.